...
def dt_diaphuong_data():
    ...
    query = """SELECT * FROM dtdl_VN_tydong WHERE Row_country IN ('Hà Nội', 'TP. Hồ Chí Minh', 'Quảng  Nam', 
    'Đà Nẵng', 'Khánh  Hòa', 'Hải Phòng', 'Thừa Thiên-Huế', 'Quảng Ninh', 'Bà Rịa - Vũng Tàu','Quảng Bình')"""
...
# return render_template('population_chart.html', province_names=province_names,
# average_populations=average_populations)
...
def population_page():
    ...
    df_population = pd.read_sql_query(
        'SELECT province_name, population_2021, population_2022, population_2023 FROM populations', conn)
    ...
    return render_template('population.html', graph_map_json=graph_map_json, graph_bar_json=graph_bar_json,
